...
-- Agents Insert and Update
CREATE POLICY "Agents can insert results" ON public.results FOR INSERT WITH CHECK (auth.uid() = submitted_by);
CREATE POLICY "Agents can update own results" ON public.results FOR UPDATE USING (auth.uid() = submitted_by);
...
(127, 3, 'Ukanafun'),
(128, 3, 'Uruan'),
(129, 3, 'Urue-Offong/Oruko'),
(130, 3, 'Uyo');

-- Seed Wards and PUs for Akwa Ibom
DO $$
DECLARE
    lga_record RECORD;
    ward_idx INT := 1;
...
BEGIN
  ...
  INSERT INTO public.users (id, email, full_name, nin, phone_number, role, assigned_pu_id)
  VALUES (agent_id, 'agent1@nems.demo', 'Chinedu Okeke', '81234567890', '+2348012345678', 'agent', 'PU-AK-0001');
  ...
  VALUES (monitor_id, 'monitor1@nems.demo', 'Aisha Bello', 'monitor');
  ...
  INSERT INTO public.users (id, email, full_name, role)
  VALUES (superadmin_id, 'superadmin@nems.demo', 'System Administrator', 'superadmin');
